...
class Coordinator {
    public:
        ...
        void set_simulator(AbcSimFun * abcsf) { _simulator = abcsf; }
        void set_simulator(AbcSimMPI * simulator) { set_simulator(ptr_simulator(simulator)); }
        void set_simulator(AbcSimBase * simulator) { set_simulator(ptr_simulator(simulator)); }
        void set_simulator(
            std::string target, const bool mpi = false, const bool executable = false
        ) {
            if (executable) {
                set_simulator(cmd_simulator(target));
            } else {
                set_simulator(SO_simulator(target, mpi));
            }
        }
        ...
        // n.b., the 1 versions are written in terms of the n versions, because efficient options to request in bulk

        // Get the parameters for a particle by serial number
        // @param serials the serial number of the particle
        std::vector<std::vector<double>> get_particle_parameters(const std::vector<size_t> &serials);

        // Get the parameters for a particle by serial number
        // @param serials the serial number of the particle
        std::vector<std::vector<double>> get_particle_metrics(const std::vector<size_t> &serials);

        // Get the parameters for a particle by serial number
        // @param serial the serial number of the particle
        std::vector<double> get_particle_parameters(const size_t serial) {
            return get_particle_parameters(std::vector<size_t>{ serial }).front();
        }

        // Get the metrics for a particle by serial number
        // @param serial the serial number of the particle
        std::vector<double> get_particle_metrics(const size_t serial) {
            return get_particle_metrics(std::vector<size_t>{ serial }).front();
        }
...
// CORE numerical bits:
        ...
        AbcSimFun * _simulator = default_simulator();
