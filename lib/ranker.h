...
/*
/ from https://sites.google.com/site/jivsoft/Home
/ 
/ Below are links to code and documents I (Ken Wilder) wrote while I was a 
/ researcher at The University of Chicago from 1997-2006 before leaving to
/ work at Google.  I do not actively maintain these projects anymore, but I 
/ continue to get emails about them and believe there is enough useful code 
/ to make them worth keeping around.  
/ 
  ...
  void get_ranks(vector<S> &w, const string &method) const {
    ...
    if (method == "average") {
      for (uint c = 0, reps; c < w.size(); c += reps) { reps = 1;
        while (c + reps < w.size() and p[tmp[c]] == p[tmp[c + reps]]) ++reps;
  ...
  void get_partial_ranks(vector<S> &w, const string &method, size_t num) const {
    ...
    if (method == "average") {
      for (uint c = 0, reps; c < num; c += reps) { reps = 1;
        while (c + reps < num and p[tmp[c]] == p[tmp[c + reps]]) ++reps;
	for (uint k = 0; k < reps; ++k)
          w[tmp[c + k]] = S(2 * c + reps - 1) / 2 + 1;
      }
    } else if (method == "min") {
